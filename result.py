...
def compile_result(
    model_args = [(5, 0.6, 1), (10, 0.3, 2), (15, 0.16, 3)],
        ...
        df = pd.DataFrame(
            {
                ...
                "fci" : fci(n, prob, hid, model, sample),
                "bpag" : bpag(n, prob, hid, model, sample),
                "bopt" : result_pag(n, prob, hid, model, sample, (min_prob, 1), skel)
            }
...
def scores_df(df, fci=False):
    ...
    df_scores = pd.concat(
        ...
        + [ df.apply(lambda x:
           ...
           for s in r],
        keys = [s + "_acc" for s in r] + [s + "_cs" for s in r],
        axis = 1
    )
